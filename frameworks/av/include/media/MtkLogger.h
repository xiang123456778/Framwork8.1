...
#define _BUILD_LOG_LEVEL 3
static int _log_level = 3;
#elif defined(CONFIG_MT_USERDEBUG_BUILD) // userdebug load
#define _BUILD_LOG_LEVEL 2
static int _log_level = 2;
#else // user load
#define _BUILD_LOG_LEVEL 1
static int _log_level = 1;
#endif // CONFIG_MT_ENG_BUILD

#define MTK_ALOGV(...) ALOGD_IF(_log_level >= 3, __VA_ARGS__) // eng
#define MTK_ALOGD(...) ALOGD_IF(_log_level >= 2, __VA_ARGS__) // userdebug/eng
#define MTK_ALOGI(...) ALOGD_IF(_log_level >= 1, __VA_ARGS__) // user/userdebug/eng
...
#define MTK_ALOGV_IF(...) if (_log_level >= 3) ALOGD_IF(__VA_ARGS__) // eng
#define MTK_ALOGD_IF(...) if (_log_level >= 2) ALOGD_IF(__VA_ARGS__) // userdebug/eng
#define MTK_ALOGI_IF(...) if (_log_level >= 1) ALOGD_IF(__VA_ARGS__) // user/userdebug/eng
...
#define MTK_MALOGV(fmt, arg...) ALOGD_IF(_log_level >= 3, "[%s]line:%d " fmt, __FUNCTION__, __LINE__, ##arg) // eng
#define MTK_MALOGD(fmt, arg...) ALOGD_IF(_log_level >= 2, "[%s]line:%d " fmt, __FUNCTION__, __LINE__, ##arg) // userdebug/eng
#define MTK_MALOGI(fmt, arg...) ALOGD_IF(_log_level >= 1, "[%s]line:%d " fmt, __FUNCTION__, __LINE__, ##arg) // user/userdebug/eng
...
#define MTK_MALOGV_IF(cond, fmt, arg...) if (_log_level >= 3) ALOGD_IF(cond, "[%s]line:%d " fmt, __FUNCTION__, __LINE__, ##arg) // eng
#define MTK_MALOGD_IF(cond, fmt, arg...) if (_log_level >= 2) ALOGD_IF(cond, "[%s]line:%d " fmt, __FUNCTION__, __LINE__, ##arg) // userdebug/eng
#define MTK_MALOGI_IF(cond, fmt, arg...) if (_log_level >= 1) ALOGD_IF(cond, "[%s]line:%d " fmt, __FUNCTION__, __LINE__, ##arg) // user/userdebug/eng
...
static void InitializeMTKLogLevel(const char * property) {
    ...
    ALOGD("%s: default level[%d]", __FUNCTION__, _log_level);
};
#else
