...
status_t AudioTrack::set(
        ...
        float maxRequiredSpeed)
{
    MTK_ALOGD("set(): %p, streamType %d, sampleRate %u, format %#x, channelMask %#x, frameCount %zu, "
    ...
    if (cbf != NULL) {
        MTK_ALOGV("set: Create AudioTrackThread");
...
{
    MTK_ALOGD("start(): %p, mState = %d", this, mState);
...
void AudioTrack::stop()
{
    ...
    MTK_ALOGD("audiotrack %p stop done", this);
}
...
{
    MTK_ALOGD("flush(): %p, mState = %d", this, mState);
    if (mSharedBuffer != 0) {
        return;
    }
    AutoMutex lock(mLock);
...
{
    MTK_ALOGD("pause(): %p, mState = %d", this, mState);
    AutoMutex lock(mLock);
    if (mState == STATE_ACTIVE) {
        mState = STATE_PAUSED;
...
void AudioTrack::setLoop_l(uint32_t loopStart, uint32_t loopEnd, int loopCount)
{
    ...
    MTK_ALOGD("setLoop_l %d %d %d",loopStart, loopEnd,  loopCount);
...
status_t AudioTrack::setMarkerPosition(uint32_t marker)
{
    ...
    MTK_ALOGD("setMarkerPosition: %p, marker = %d", this, marker);
    AutoMutex lock(mLock);
...
status_t AudioTrack::getMarkerPosition(uint32_t *marker) const
{
    ...
    MTK_ALOGD("getMarkerPosition: %p, marker = %d", this, *marker);
...
status_t AudioTrack::setPositionUpdatePeriod(uint32_t updatePeriod)
{
    ...
    MTK_ALOGD("setPositionUpdatePeriod: %p, updatePeriod = %d", this, updatePeriod);
    AutoMutex lock(mLock);
...
status_t AudioTrack::getPosition(uint32_t *position)
{
    ...
    MTK_ALOGV("getPosition: %p, *position = %d", this, *position);
    return NO_ERROR;
}
...
status_t AudioTrack::createTrack_l()
{
    ...
    MTK_ALOGD("createTrack_l: %p, mCblk = %p, mLatency = %d, mAfLatency = %d, frameCount = %zu, mSampleRate = %d, mFlags = %#x, mReqFrameCount = %zu",
            this, mCblk, mLatency, mAfLatency, frameCount, mSampleRate, mFlags, mReqFrameCount);
...
nsecs_t AudioTrack::processAudioBuffer()
{
    ...
    MTK_ALOGV("processAudioBuffer(): ns = %" PRId64, ns);
...
Modulo<uint32_t> AudioTrack::updateAndGetPosition_l()
{
    ...
    MTK_ALOGD("updateAndGetPosition_l: %p, mPosition, mServer, delta = %d, %d, %d",
...
{
    MTK_ALOGD("AudioTrackThread::pause");
    AutoMutex _l(mMyLock);
    mPaused = true;
}
...
{
    MTK_ALOGD("AudioTrackThread::resume");
    AutoMutex _l(mMyLock);
...
{
    MTK_ALOGV("AudioTrackThread::pauseInternal: ns = %" PRId64, ns);
