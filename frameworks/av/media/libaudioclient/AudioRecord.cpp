        ...
        const audio_attributes_t* pAttributes)
{
    ...
    if (cbf != NULL) {
        MTK_ALOGD("set: Create AudioRecordThread");
...
{
    MTK_ALOGD("start: %p, sync event %d trigger session %d", this, event, triggerSession);
    ...
    MTK_ALOGD("return status %d", status);
    return status;
}
...
status_t AudioRecord::getTimestamp(ExtendedTimestamp *timestamp)
{
    ...
    if (status == OK) {
        ...
        for (int i = ExtendedTimestamp::LOCATION_SERVER;
            ...
            MTK_ALOGV("%s: i = %d, time = %" PRId64" , position = %" PRId64, __FUNCTION__, i, timestamp->mTimeNs[i],
...
status_t AudioRecord::openRecord_l(const Modulo<uint32_t> &epoch, const String16& opPackageName)
{
    ...
    mCblk = cblk;
    MTK_ALOGD("openRecord_l: %p, mCblk = %p", this, mCblk);
...
nsecs_t AudioRecord::processAudioBuffer()
{
    ...
    // FIXME fails for wraparound, need 64 bits
    MTK_ALOGV("updatePeriod %d, position %u >= newPosition %u",
