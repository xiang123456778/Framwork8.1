...
#if defined(MTK_FM_SUPPORT) || defined(MTK_TTY_SUPPORT) || defined(MTK_AUDIO_GAIN)
...
static String8 keyhifiaudio =String8("audio_samplerate");
#endif
...
status_t AudioSystem::setParameters(audio_io_handle_t ioHandle, const String8& keyValuePairs)
{
    ...
    if (param.getInt(keyhifiaudio, sampleratevalue) == NO_ERROR) {
        const sp<IAudioPolicyService>& aps = AudioSystem::get_audio_policy_service();
        if (aps != 0) {
            ret = aps->setPolicyManagerParameters (POLICY_SET_AUDIO_RATE, sampleratevalue, 0, 0);
        }
        return ret;
    }
#endif
