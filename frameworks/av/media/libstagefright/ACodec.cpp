...
status_t ACodec::setupMtkRawCodec(int32_t numChannels, int32_t sampleRate, const sp<AMessage> &msg) {
    ...
    err = mOMXNode->getParameter((OMX_INDEXTYPE)OMX_IndexParamAudioRaw, &def, sizeof(def));
    if (err != OK) {
        return err;
    }
    ...
    ALOGD("endian is %d, bitWidth is %d, pcmType is %d, channelAssignment is %d, numericalType is %d",
                    endian, bitWidth, pcmType, channelAssignment, numericalType);
    ...
    return mOMXNode->setParameter((OMX_INDEXTYPE)OMX_IndexParamAudioRaw, &def, sizeof(def));
}
#endif
