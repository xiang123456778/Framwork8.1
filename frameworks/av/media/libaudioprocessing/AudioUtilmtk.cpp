...
namespace android {
KeyedVector<String8, int > mAudioDumpKeyEnableVector;
...
void AudioDump::dump(const char * filepath, void * buffer, int count,const char * property)
{
    { // mAudioDumpKeyMutex region
        ...
        ssize_t index = mAudioDumpKeyEnableVector.indexOfKey(String8(property));
        if (index < 0) { // new add
            int bflag = (int)property_get_bool(property, false);
            mAudioDumpKeyEnableVector.add(String8(property), bflag);
            if (!bflag) {
                return;
            }
        } else if (!mAudioDumpKeyEnableVector[index]) {
            return;
        }
        index = mAudioDumpKeyEnableVector.indexOfKey(String8(af_dump_log));
        if (index < 0) { // new add
            int bflag = (int)property_get_bool(af_dump_log, false);
            mAudioDumpKeyEnableVector.add(String8(af_dump_log), bflag);
        }
    }
    ...
    if (ret < 0) {
        ...
        if (fp != NULL) {
            ...
            fclose(fp);
            if (mAudioDumpKeyEnableVector.valueFor(String8(af_dump_log)) > 0) {
...
void AudioDump::updateKeys(const char * property)
{
    Mutex::Autolock _l(mAudioDumpKeyMutex);
    ...
    if (property == 0) {
        ...
        mAudioDumpKeyEnableVector.removeItem(String8(property));
    }
}
