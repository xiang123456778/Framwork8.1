...
#if defined(MTK_AUDIO_DEBUG)
            String8 fileName = String8::format("%s_pid%d_tid%d_%p.pcm", gaf_mixer_end_pcm,
                getpid(), gettid(), out);
            AudioDump::threadDump(fileName, out,
                BLOCKSIZE * t1.mMixerChannelCount * audio_bytes_per_sample(t1.mMixerFormat),
                AudioParameter::keyAudioDumpMixerEnd, t1.mMixerFormat, t1.sampleRate,
...
void AudioMixer::process__genericResampling(state_t* state)
{
    ...
    while (e0) {
        ...
        AudioDump::threadDump(fileName, out,
            ...
            AudioParameter::keyAudioDumpMixerEnd, t1.mMixerFormat, t1.mDevSampleRate,
            t1.mMixerChannelCount);
#endif // MTK_AUDIO_DEBUG
    }
}
...
void AudioMixer::process_NoResampleOneTrack(state_t* state)
{
    ...
    for (size_t numFrames = state->frameCount; numFrames; ) {
        ...
        AudioDump::threadDump(fileName, out,
            ...
            AudioParameter::keyAudioDumpMixerEnd, t->mMixerFormat, t->sampleRate,
            t->mMixerChannelCount);
#endif // MTK_AUDIO_DEBUG
...
#if defined(MTK_AUDIO_DEBUG)
    String8 fileName = String8::format("%s_pid%d_tid%d_%p.pcm", gaf_mixer_drc_pcm_before,
        getpid(), gettid(), ioBuffer);
    AudioDump::threadDump(fileName, ioBuffer, SampleSize, AudioParameter::keyAudioDumpDrc,
...
#if defined(MTK_AUDIO_DEBUG)
    fileName = String8::format("%s_pid%d_tid%d_%p.pcm", gaf_mixer_drc_pcm_after,
        getpid(), gettid(), tempBuffer);
    AudioDump::threadDump(fileName, tempBuffer, SampleSize, AudioParameter::keyAudioDumpDrc,
        process_format, mDevSampleRate, process_channel);
#endif // MTK_AUDIO_DEBUG
