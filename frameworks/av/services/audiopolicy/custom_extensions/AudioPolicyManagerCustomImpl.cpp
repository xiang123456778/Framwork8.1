...
#if defined(MTK_AUDIO_DEBUG)
#define MTK_ALOGVV ALOGD
#define MTK_ALOGV ALOGD
#else
#define MTK_ALOGVV ALOGVV
#define MTK_ALOGV ALOGV
#endif
...
AudioPolicyManagerCustomImpl::AudioPolicyManagerCustomImpl()
{
    ...
    mAudioPolicyManager->mAudioPolicyVendorControl.setPrevMode(AUDIO_MODE_NORMAL);
#endif
}
...
status_t AudioPolicyManagerCustomImpl::gainTable_getCustomAudioVolume(void)
{
    ...
    if (mAudioPolicyManager->mpClientInterface->getCustomAudioVolume(&mGainTable)) {
...
audio_devices_t AudioPolicyManagerCustomImpl::fm_correctDeviceFromSetDeviceConnectionStateInt(const sp<AudioOutputDescriptor>& outputDesc, audio_devices_t device, bool force)
{
    ...
    if (outputDesc == mAudioPolicyManager->mPrimaryOutput && force) {
...
status_t AudioPolicyManagerCustomImpl::common_setPolicyManagerCustomParameters(int par1, int par2, int par3, int par4)
{
    ...
    ALOGD("setPolicyManagerCustomParameters par1 = %d par2 = %d par3 = %d par4 = %d curDevice = 0x%x", par1, par2, par3, par4, curDevice);
    status_t volStatus;
    switch(par1) {
        ...
        case POLICY_SET_FM_PRESTOP: {
            ...
            for (size_t i = 0; i < mAudioPolicyManager->mOutputs.size(); i++) {
                ...
                if (desc->sharesHwModuleWith(mAudioPolicyManager->mPrimaryOutput) && !desc->isDuplicated()) {
                    if (par2) {
        ...
        case POLICY_SET_AUDIO_RATE:{
            ...
            for (size_t i = 0; i < mAudioPolicyManager->mOutputs.size(); i++) {
                ...
                if (!outputDesc->isDuplicated() &&  outputDesc->sharesHwModuleWith(mAudioPolicyManager->mPrimaryOutput)&& !(outputDesc->mFlags & AUDIO_OUTPUT_FLAG_FAST)) {
            ...
            /*
             * base on smaple rate to set FM indirect mode
             * then tell fm apk to restart
            */
...
float AudioPolicyManagerCustomImpl::computeGainTableCustomVolume(audio_stream_type_t stream, int index, audio_devices_t device)
{
    ...
    if (deviceCategory == DEVICE_CATEGORY_SPEAKER) {
        ...
        } else if (device & AUDIO_DEVICE_OUT_USB_PRIMARY) {
                                                           ...
                                                           int delayMs, bool force)
{
    ...
    if (mVolumeStream != activeStream || mVolumeIndex != index || mVolumeDevice != device || force ) {
        ALOGD("computeAndSetAnalogGain stream %d(%d), device 0x%x(0x%x), index %d(%d)", activeStream, mVolumeStream, device, mVolumeDevice, index, mVolumeIndex);
...
status_t AudioPolicyManagerCustomImpl::fm_addAudioPatch(audio_patch_handle_t handle, const sp<AudioPatch>& patch)
{
    ...
    bool bFMeable = false;
    sp<SwAudioOutputDescriptor> outputDesc = mAudioPolicyManager->mPrimaryOutput;
...
#if defined(MTK_FM_SUPPORT)
    (void) force;
    if (!mSkipFMVolControl && stream == AUDIO_STREAM_MUSIC && outputDesc == mAudioPolicyManager->mPrimaryOutput && (device & (AUDIO_DEVICE_OUT_WIRED_HEADSET | AUDIO_DEVICE_OUT_WIRED_HEADPHONE | AUDIO_DEVICE_OUT_SPEAKER))) {
...
status_t AudioPolicyManagerCustomImpl::fm_muteStrategyFromCheckOutputForStrategy(routing_strategy strategy, audio_devices_t oldDevice, audio_devices_t newDevice)
{
    ...
    if (strategy == STRATEGY_MEDIA) {
        ...
        if ((oldDevice & (~(AUDIO_DEVICE_OUT_SPEAKER|AUDIO_DEVICE_OUT_WIRED_HEADSET|AUDIO_DEVICE_OUT_WIRED_HEADPHONE)))
            ...
            ALOGD("FM mute in-direct primary first, oldDevice 0x%x -> newDevice 0x%x", oldDevice, newDevice);
            mAudioPolicyManager->setStrategyMute(strategy, true, mAudioPolicyManager->mPrimaryOutput);
            mAudioPolicyManager->setStrategyMute(strategy, false, mAudioPolicyManager->mPrimaryOutput, MUTE_TIME_MS, newDevice);
        }
    }
    return NO_ERROR;
#else
...
{
#if defined(MTK_FM_SUPPORT)
    if (outputDesc == mAudioPolicyManager->mPrimaryOutput && strategy == STRATEGY_MEDIA && isFMActive() && muteDurationMs < extendDurationMs) {
                                                                                      ...
                                                                                      const char *device_name)
{
    ...
    if (device == AUDIO_DEVICE_OUT_USB_DEVICE || device == AUDIO_DEVICE_OUT_USB_HEADSET) {
        autodevice = AUDIO_DEVICE_OUT_USB_PRIMARY;
    } else if (device == AUDIO_DEVICE_IN_USB_DEVICE || device == AUDIO_DEVICE_IN_USB_HEADSET) {
        autodevice = AUDIO_DEVICE_IN_USB_PRIMARY;
    }
    ...
    if (autodevice != AUDIO_DEVICE_NONE && status == NO_ERROR) {
        ...
        mAudioPolicyManager->setDeviceConnectionStateInt(autodevice, state, "", "");
    }
    return status;
#else
                                             ...
                                             uint32_t *muteWaitMs)
{
    ...
    if (rxDevice == AUDIO_DEVICE_OUT_USB_PRIMARY) {
        ...
        *txDevice = mAudioPolicyManager->getDeviceAndMixForInputSource(AUDIO_SOURCE_VOICE_COMMUNICATION);
        if (usbOutputChanged && *txDevice == AUDIO_DEVICE_IN_USB_PRIMARY) {
...
status_t AudioPolicyManagerCustomImpl::usbPhoneCall_addCurrentVolumeIndexFromSetStreamVolumeIndex(audio_stream_type_t stream,
                                                  int index,
                                                  audio_devices_t device)
{
#if defined(MTK_USB_PHONECALL)
    if ((device == AUDIO_DEVICE_OUT_USB_DEVICE && !mUsbHeadsetConnect) ||
        ...
        device |= AUDIO_DEVICE_OUT_USB_PRIMARY;
        for (int curStream = 0; curStream < AUDIO_STREAM_FOR_POLICY_CNT; curStream++) {
            if (!mAudioPolicyManager->streamsMatchForvolume(stream, (audio_stream_type_t)curStream)) {
                continue;
            }
            mAudioPolicyManager->mVolumeCurves->addCurrentVolumeIndex((audio_stream_type_t)curStream, AUDIO_DEVICE_OUT_USB_PRIMARY, index);
        }
    }
    return NO_ERROR;
#else
...
audio_devices_t AudioPolicyManagerCustomImpl::usbPhoneCall_correctDeviceFromGetDevicesForStream(audio_devices_t devices)
{
    ...
    if (devices & AUDIO_DEVICE_OUT_USB_PRIMARY) {
        if (!mUsbHeadsetConnect) {
            devices |= AUDIO_DEVICE_OUT_USB_DEVICE;
        } else {
            devices |= AUDIO_DEVICE_OUT_USB_HEADSET;
        }
        devices &= ~AUDIO_DEVICE_OUT_USB_PRIMARY;
    }
    return devices;
#else
    return devices;
...
{
#if defined(MTK_AUDIO_DEBUG)
    ALOGD("getOutputForAttr() device 0x%x, sample_rate %d, format %x, channel_mask %x, flags %x stream %d, output %d",
...
{
#if defined(MTK_AUDIO_DEBUG)
    ALOGD("getInputForAttr() returns input type = %d device =0x%x *input = %d", inputType, device, input);
    return true;
#else
    (void) inputType;
    (void) device;
    ...
    for (size_t i = 0; i < mAudioPolicyManager->mOutputs.size(); i++) {
        ...
        if (outputDesc == outputdesc) {
            if (device == AUDIO_DEVICE_NONE && !force) {
                ...
                ALOGD("setOutputDevice() mIoHandle %d mId %d device %04x(%04x)(%04x) delayMs %d force %d size %zu", mAudioPolicyManager->mOutputs.keyAt(i), outputDesc->getId(), device, outputDesc->mDevice, outputDesc->supportedDevices(), delayMs, force, mAudioPolicyManager->mOutputs.size());
            }
            break;
        }
...
bool AudioPolicyManagerCustomImpl::setFMIndirectMode(uint32_t sampleRate)
{
    ...
    if (sampleRate > 48000) {
        ...
        mAudioPolicyManager->mpClientInterface->setParameters(mAudioPolicyManager->mPrimaryOutput->mIoHandle, param.toString(),0);
        return true;
    } else {
...
bool AudioPolicyManagerCustomImpl::checkUsbSuspend(audio_devices_t device)
{
    ...
    if (mAudioPolicyManager->mAudioPolicyVendorControl.getUsbSuspended()) {
        if (mAudioPolicyManager->mPrimaryOutput->device() != AUDIO_DEVICE_OUT_USB_PRIMARY ||
        ...
        if ((mAudioPolicyManager->mPrimaryOutput->device() == AUDIO_DEVICE_OUT_USB_PRIMARY || device == AUDIO_DEVICE_OUT_USB_PRIMARY) &&
            mAudioPolicyManager->mEngine->getPhoneState() == AUDIO_MODE_IN_CALL) {
