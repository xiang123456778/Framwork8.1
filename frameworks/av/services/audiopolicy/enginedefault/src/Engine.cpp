...
#if defined(MTK_USB_PHONECALL)
            if (getPhoneState() == AUDIO_MODE_IN_CALL ||
                (getPhoneState() == AUDIO_MODE_IN_COMMUNICATION && mAudioPolicyVendorControl.getUsbSuspended())) {
                device = availableOutputDevicesType & AUDIO_DEVICE_OUT_USB_PRIMARY;
                if (device) break;
            }
#endif
...
#if defined(MTK_USB_PHONECALL)
        if (device2 == AUDIO_DEVICE_NONE && (getPhoneState() == AUDIO_MODE_IN_CALL ||
            (getPhoneState() == AUDIO_MODE_IN_COMMUNICATION && mAudioPolicyVendorControl.getUsbSuspended()))) {
            device2 = availableOutputDevicesType & AUDIO_DEVICE_OUT_USB_PRIMARY;
        }
#endif
        if (device2 == AUDIO_DEVICE_NONE) {
...
#if defined(MTK_USB_PHONECALL)
    } else if ((availableDeviceTypes & AUDIO_DEVICE_IN_USB_PRIMARY) && mAudioPolicyVendorControl.getUsbSuspended()) {
        device = AUDIO_DEVICE_IN_USB_PRIMARY;
#endif
    ...
    case AUDIO_SOURCE_VOICE_COMMUNICATION:
        ...
        default:    // FORCE_NONE
            if (availableDeviceTypes & AUDIO_DEVICE_IN_WIRED_HEADSET) {
                device = AUDIO_DEVICE_IN_WIRED_HEADSET;
#if defined(MTK_USB_PHONECALL)
            } else if ((availableDeviceTypes & AUDIO_DEVICE_IN_USB_PRIMARY) &&
                            mAudioPolicyVendorControl.getUsbSuspended()) {
                device = AUDIO_DEVICE_IN_USB_PRIMARY;
#endif
            } else if (availableDeviceTypes & AUDIO_DEVICE_IN_USB_HEADSET) {
                device = AUDIO_DEVICE_IN_USB_HEADSET;
