...
void AudioFlinger::ThreadBase::TrackBase::releaseBuffer(AudioBufferProvider::Buffer* buffer)
{
    ...
    if (!mIsOut) {
        String8 fileName;
        fileName = String8::format("%s_%d_%p_in.pcm", gaf_track_pcm, mTrackCount, this);
        AudioDump::threadDump(fileName, buffer->raw, buffer->frameCount * mFrameSize,
        ...
        AudioBufferProvider::Buffer* buffer)
{
    ...
    if (NULL == buffer->raw) {
        ...
        if (mIsOut) {
            String8 fileName;
            fileName = String8::format("%s_%d_%p_out.pcm", gaf_track_pcm, mTrackCount, this);
            AudioDump::threadDump(fileName, buffer->raw, buffer->frameCount * mFrameSize,
                AudioParameter::keyAudioDumpTrack, mFormat, mSampleRate, mChannelCount);
        }
    }
#endif // MTK_AUDIO_DEBUG
