...
#if defined(MTK_AUDIO_DEBUG)
            String8 fileName = String8::format("%s_session%d_%d_in.pcm",
                gaf_effect_pcm, mSessionId, mId);
            AudioDump::threadDump(fileName, mConfig.inputCfg.buffer.s16,
                mConfig.inputCfg.buffer.frameCount * FCC_2
                * audio_bytes_per_sample((audio_format_t)mConfig.inputCfg.format),
                AudioParameter::keyAudioDumpEffect, (audio_format_t)mConfig.inputCfg.format,
                mConfig.inputCfg.samplingRate, FCC_2);
#endif // MTK_AUDIO_DEBUG
            ret = mEffectInterface->process();
#if defined(MTK_AUDIO_DEBUG)
            fileName = String8::format("%s_session%d_%d_out.pcm",
                gaf_effect_pcm, mSessionId, mId);
            AudioDump::threadDump(fileName, mConfig.outputCfg.buffer.s16,
                mConfig.outputCfg.buffer.frameCount * FCC_2
                * audio_bytes_per_sample((audio_format_t)mConfig.outputCfg.format),
                AudioParameter::keyAudioDumpEffect, (audio_format_t)mConfig.outputCfg.format,
                mConfig.outputCfg.samplingRate, FCC_2);
#endif // MTK_AUDIO_DEBUG
        } else {
