...
public class WifiStateMachine extends StateMachine implements WifiNative.WifiRssiEventHandler,
    ...
    /// M: for IP recover  @{
    private static HashMap<String, DhcpResults> sDhcpResultMap = new HashMap<String, DhcpResults>();
    /// @}
    ...
    private void handleIPv4Success(DhcpResults dhcpResults) {
        ...
        synchronized (mDhcpResultsLock) {
            ...
            if (wifiCfg != null) {
                ...
                sDhcpResultMap.put(ssid, new DhcpResults(dhcpResults));
            }
            /// @}
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                case DhcpClient.CMD_PRE_DHCP_ACTION:
                    ...
                    break;
                case DhcpClient.CMD_PRE_DHCP_ACTION_COMPLETE:
                    /// M: ALPS03051043: IP recover switch  @{
                    // mIpManager.completedPreDhcpAction();
                    WifiConfiguration wifiConfig = getCurrentWifiConfiguration();
                    boolean isIpRecoverEnabled = true;
                    // is current wifi ap authentication WEP-OPEN?
                    if (wifiConfig != null &&
                        ...
                        isIpRecoverEnabled = false;
                    }
                    mIpManager.completedPreDhcpAction(isIpRecoverEnabled);
                    /// @}
                    break;
                case DhcpClient.CMD_POST_DHCP_ACTION:
                    handlePostDhcpSetup();
    ...
    class ObtainingIpState extends State {
        ...
        public void enter() {
            ...
            if (!isUsingStaticIp) {
                ...
                if (wifiCfg != null) {
                    String ssid = wifiCfg.getPrintableSsid();
                    DhcpResults record = sDhcpResultMap.get(ssid);
                    logd("IP recover: get DhcpResult for ssid-->" + ssid + ", record-->" + record);
                    mIpManager.updatePastSuccessedDhcpResult(record);
                }
                /// @}
